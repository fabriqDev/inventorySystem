...
export function toBackendError(error: unknown): BackendError {
  ...
  if (parsed.type === 'PERMISSION_DENIED' || parsed.type === 'FORBIDDEN') {
    return new BackendError({
      message: parsed.detail ?? 'You do not have permission for this action.',
      kind: 'unauthorized',
      code: parsed.code,
      type: parsed.type,
      detail: parsed.detail,
  ...
  if (parsed.type === 'CONFLICT') {
    return new BackendError({
      message: parsed.detail ?? 'A conflicting operation occurred. Please try again.',
      kind: 'conflict',
      code: parsed.code,
      type: parsed.type,
      detail: parsed.detail,
...
export function toUserMessage(err: BackendError): string {
  if (err.kind === 'validation' || err.kind === 'resource_not_found' || err.kind === 'conflict') {
    return err.message;
  }
