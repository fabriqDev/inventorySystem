...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const addItem = useCallback((product: Product, options?: { transactionType?: CartTransactionType }) => {
    ...
    const code = product.article_code;
    setItems((prev) => {
      const existing = prev.find((i) => i.article_code === code && i.transactionType === transactionType);
      if (existing) {
        return prev.map((i) =>
          i.article_code === code && i.transactionType === transactionType
  ...
  const removeItem = useCallback((productId: string, transactionType?: CartTransactionType) => {
    ...
    setItems((prev) =>
      prev.filter((i) => !(i.article_code === productId && i.transactionType === type)),
    );
  }, []);
  ...
  const updateQuantity = useCallback((productId: string, quantity: number, transactionType?: CartTransactionType) => {
    ...
    setItems((prev) =>
      prev.map((i) =>
        i.article_code === productId && i.transactionType === type ? { ...i, quantity } : i,
      ),
    );
  }, []);
